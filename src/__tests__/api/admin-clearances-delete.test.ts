...
const { DELETE } = await import(
  "@/app/api/admin/users/[userId]/clearances/[sectorId]/route"
);

describe("DELETE /api/admin/users/[userId]/clearances/[sectorId]", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("returns 403 when not admin", async () => {
    ...
    const res = await DELETE(new Request("http://localhost"), {
      params: Promise.resolve({ userId: "u1", sectorId: "sec-cyber" }),
    });
  ...
  it("deletes the sector grant and returns {deleted:true}", async () => {
    mockAuth.mockResolvedValue(mockSession({ id: "admin-id", role: "admin" }));
    ...
    expect(mockPrisma.userClearance.delete).toHaveBeenCalledWith({
      where: {
        userId_sectorId: { userId: "u1", sectorId: "sec-cyber" },
      },
    });
  });
});
